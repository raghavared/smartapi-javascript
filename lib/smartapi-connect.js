...
var SmartApi = function (params) {
	...
	self.searchScrip = function (params) {
		const res = post_request('search_scrip', params).then((data) => {
			if (data?.status === true && data?.data?.length > 0) {
				const searchData = {
					msg: `Search successful. Found ${data.data.length} trading symbols for the given query:\n`
				};
				...
				searchData.msg += tradingSymbols.join('\n'); 
				return searchData;
			}
